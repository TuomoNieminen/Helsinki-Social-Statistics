...
One of R's data types is called factor. In factors you have numeric values, but for every number there is a character value associated to it. [Likert scale](https://en.wikipedia.org/wiki/Likert_scale) could be an example of a factor: there are numbers from 1 to 5, but the numbers have a unique words attached to them.
...
# Print the first 6 elements of student_age_cut
...
test_function("head", incorrect_msg = "Did you use the `head()` to look at the first six elements of `student_age_cut`?")
...
A vector is an object containing multiple elements. All the elements need to be from same data type. Vectors are quite important in R. The columns of a data frame (such as `student2014`) are vectors.
...
Use the R Console to inspect the `students2014` data and select the correct answer. The data is already loaded for you. Remember that in console `Enter` executes lines.
...
- You can put the values in a meaningful order, then it's ordinal scale. 
- Factors create a unique encoding for all the different values. Using factors loses its purpose when you have a lot of different values.
