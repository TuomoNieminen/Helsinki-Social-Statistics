...
When creating character values, one needs to use quotation marks `' '`. For example `'Statistics<3'` would be a character value in R. Numeric and boolean values do not need quotation marks. 
...
test_object("names <- c('Matti', 'Pekka', 'Liisa', 'Anna')", incorrect_msg = "Did you execute `names`?")
test_object("girls <- names[c(3, 4)]", incorrect_msg = "Did you create `girls`?")
test_object("boys <- names[c(2, 1)]", incorrect_msg = "Did you create `boys`?")
...
*** =sct
```{r}
test_object("my_vector <- c(2, 3, 1, 5)", incorrect_msg = "Did you correct the `my_vector`?")
...
# Measurement
hist(students2014$ika, breaks = 50)

# From measurement to ordinal
student_age_cut <- cut(students2014$ika, breaks = c(15, 25, 35, 45, 55, 65, 75, 85))
