...
with(students2014, 
     plot(kenka, pituus, col = sukup, pch = 20))
```
...
with(students2014, 
     plot(x = kenka, y = pituus, col = sukup, pch = 20))
...
The way to use these two is to first write code to the editor area and then to tell R that you want to execute a line of code by pressing `Ctrl + Enter`. Input and output will then appear in the console, executing the line where your cursor currently is.

It is also possible to write code directly to the console and use `Enter` to execute, but working with the script area is preferred for documentation.

Here at DataCamp you can also execute everything in the editor area by pressing 'Submit Answer', after which DataCamp will do some magic to check if you have correctly completed the exercise.
...
add_k(5, -4)     # style not recommended
add_k(k = -4, value = 5) # style not recommended
