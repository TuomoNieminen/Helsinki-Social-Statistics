...
test_error()
success_msg("You're learning well, padawan.")
```
...
success_msg(paste0("Great work ", my_name, ". ", my_message, "!"))
```
...
--- type:NormalExercise lang:r xp:100 skills:1 key:79950c6be7
## Good arguments

During this course you will operate on many of R's ready-made functions. As we already saw, functions can take objects as arguments (inside the parenthesis) and then perform actions using the objects. Using a function to produce an output is called *calling* a function.
...
Multiple arguments are always separated with a comma.  
...
*** =hint
- Remember to separate the function arguments with a comma
...
*** =sample_code
```{r}
# students2014 data object is available

# Use the function head() on the object students2014
head(students2014, n = 2)
...
head(x = students2014, 3)     # style not recommended
head(n = 3, x = students2014) # style not recommended
...
test_object("first_ten_students", incorrect_msg = "Plead use `head()` with the correct arguments to create the object `first_ten_students`.")
