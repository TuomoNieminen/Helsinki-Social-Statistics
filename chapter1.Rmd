...
# Load data from the web using the function read.table(). Store the data in the object 'students2014'.
data_url <- "http://www.helsinki.fi/~kvehkala/JYTmooc/JYTOPKYS-data.txt"
students2014 <- read.table(data_url, sep = "\t", header = TRUE)

# Draw a graph of shoesize versus hight using the function plot(). Indicate sex by color by using the 'col' argument of plot().
with(students2014, 
...
In R you operate on objects using *functions*. A function is a special kind of object that can be *called* by typing it's name and giving it the necessary *arguments* inside parenthesis. When a function is called, it will perform an action using it's arguments and usually output something.  

Functions are how you're actually going to do all the magic. During this course you will operate on ready-made functions, but for clarity we will look at the definition of a simple function.
