...
*** =instructions
- Draw a *density* histogram of variable `deep` from the `learning2014` data. You can plot a density plot instead of frequency one by setting the argument `freq` to `FALSE` in the histogram plot function. Do not use the argument `breaks`. We can see that the distribution of variable deep resembles a normal distribution. 
- If we wanted to know what is the probability that deep learning is smaller than 3?
...
# Histogram of 'deep'
hist(learning2014$deep, freq = F)

# The mean and sd of 'deep'
deep_mean <- mean(learning2014$deep)
deep_sd <- sd(learning2014$deep)

# Calcu

dnorm(3, mean = deep_mean, sd = deep_sd)
deep_sequence <- seq(0, 5, length = 50) # this creates a vector that has 50 values from 0 to 5
density<-dnorm(deep_sequence, mean = deep_mean, sd = deep_sd)

draw<-function(x){
  plot(deep_sequence, density, type = 'l', col = 'red', lwd = 2, xlim=c(0,5))
  y<-dnorm(deep_sequence[deep_sequence<=x], mean = deep_mean, sd = deep_sd)
  polygon(c(0, deep_sequence[deep_sequence<=x], x), c(0, y,0), col = "orange")
}

draw(5)

pnorm(5, mean = deep_mean, sd = deep_sd, lower.tail = TRUE)
pnorm(0, mean = deep_mean, sd = deep_sd, lower.tail = FALSE)
...
$$z = \frac{\mu - \overline{x}}{\sigma}$$

*** =instructions
- Let's create a function that standardizes normal distributions. Save the function as `standarzation`.
- instruction 2
