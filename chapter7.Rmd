...
- Draw another sample, this time with sample size = `100`. Add the argument `replace = TRUE` to the `sample()` function. What do you think it the `replace` argument does? Save the sample to `my_sample`. 
...
```{r}
# submission correctness tests
test_object("normal_mean", incorrect_msg = "Did you save the mean of `normal_sample` to `normal_mean`?")
test_object("normal_sd", incorrect_msg = "Did you save the standard deviation of `normal_sample` to `normal_sd`?")
...
The function `pnorm()` can be used to calculate probabilities from normal distributions. Let's see how it works!
...
- You can access a variable of a dataset with the $-sign.
- Remember to use the `lower.tail` argument in the `pnorm()` function. It determines which "side" you are calculating.
- You can do calculations with the `pnorm()` function. 
...
*** =instructions
- Let's say that we know a probability of buing a t-shirt from store is 0.3. There are 8 people in the store looking around. What is the probability of two of them buing a t-shirt? 
- What is the probability of seven of them buing a t-shirt? Save the answer to object `seven_shirts_sold`.
- Probability of buying a t-shirt is saved in the `binom_prob` object. The plot function below visualizes the binomial distribution with that probability. Try and change the probability to 0.4, 0.5, ... and draw a plot to see what happens to the distribution. Is the distribution symmetric at some point? 
- Some extra information: the function `plot()` has arguments `lwd` which determines the line weight in the plot. The argument `col` is used to specify the color of the lines and `type` has several values to choose from which all produce a different kind of plot. See `?plot` to see the different types options.
...
- Try different values to `binom_prob` and execute the `plot()` function after changing the value. Remember to execute the code for `binom_prob` also before drawing the plot.
- Probabilities need to be decimal numbers between 0-1 (e.g. 0.7).
- You don't need to do anything for the `plot()` function.
...
# See what happens to the distribution if you change the probability
binom_prob <- 0.3

# Visualization (you don't need to do anything for this code). 
plot(dbinom(0:8, size = 8 , prob = binom_prob), type="h", lwd=2, col="steel blue")
...
seven_shirts_sold <- dbinom(7, size = 8, prob = 0.3)
...
```{r}
# submission correctness tests
test_object("seven_shirts_sold", incorrect_msg = "Did you save the probability of 7 of them buing a t-shirt into `seven_shirts_sold`?")
...
--- type:NormalExercise lang:r xp:100 skills:1 key:9a242bf8b4
## Quantiles
...
A quantile function takes a probability value as input and produces a quantile value as output. The quantile value is a point at which the probability that the random variable takes on a value less than or equal to that point, is equal to the given probability.

As with all the *norm() functions, qnorm() has default values for the arguments mean = 0 and sd = 1; corresponding to the standard normal distribution.
...
*** =instructions
- These questions relate to a standard normal variable
...
--- type:NormalExercise lang:r xp:100 skills:1 key:cced95c538
...
*** =instructions
- Execute the example codes. What is the relationship between the computations and the resulting plot?
- Compute the probability $2 \cdot P(Z < - 2.58)$
- Compute the critical value z corresponding to probability $\alpha$ = 0.01 divided to both "tails" of the distribution
- Visualize the computation

*** =hint
- Follow the examples and use the same functions
...
test_object("my_good_apples", incorrect_msg ="Did you save results of `good_apples_count()` to `my_good_apples`?")
...
To standardize multiple variables you will need to calculate means and standard deviations for all them. You could do it one variable at the time, but it takes time and the same function codes are repeated. In these kind of cases, it is useful to create one function that does the functionality we want and use it instead.

*** =instructions
- Draw first a histogram of `deep` from the `learning2014`. Note the x-axis of the histogram: the numbers are deep learning scores and you can see how they are distributed in the data.
