...
msg2 <- "This is a common misinterpretation. Any effect, no matter how tiny, can produce a significant p-value if the sample size or measurement precision is high enough, and large effects may produce unsignificant p-values if the sample size is small or measurements are imprecise."
...
- Compute the test statistic $z$. If `mu0` is the actual expected value, the random variable $Z$ corrspoinding the value $z$ follows the standard normal distribution: $Z \dis N(0,1)$
- Print out the value of the test statistic
- Visualize the distribution. According to the picture and assuming that $Z \dis N(0,1)$, what is the probability that the test statistic $Z$ would take on a value even further away from it's expected value than the current value $z$? Save this probability to the object `p`.

*** =hint
- How much probability in total is at the "tails"" of the distribution?
