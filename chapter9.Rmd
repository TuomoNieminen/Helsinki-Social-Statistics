...
regions <- function(a, direction) {
  ...
  if (direction == 'l') {
    ...
    ab
    title("Plot 1", adj=0)
    text(-2.5, .1, paste(a*100, "%"), col="steelblue")
    }
  if (direction == 'r') {
    q1 <- qnorm(1 - a)
    x1 <- x[x >= q1]
  ...
  if (direction == 'm') {
    ...
    ab
    title("Plot 3", adj=0)
    text(c(-2.5,2.5), c(.1,.1), paste((a/2)*100, "%"), col="steelblue")
  }
}
...
List of available CRAN packages can be seen [here](https://cran.r-project.org/web/packages/available_packages_by_name.html).

If you have R in your own computer, you can install packages with code `install.packages(*name_of_the_package*)`. But because we are working in DataCamp, the packages are already installed for you.
...
- Create the `grades` object and draw a plot of `grades` and `attitude`. Set `grades` as a legend.
