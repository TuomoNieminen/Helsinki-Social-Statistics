...
After setting the hypotheses, we can use the data we have and calculate a test statistic. What test statistic to use, depends on the test you are doing.

*** =instructions
- Create object `stra`
- Set the null hypothesis that the expected value of strategic learning is 3 and create object `mu0`
- Compute the point estimate of the expected value and the standard error
- Compute the test statistic $z$. If `mu0` is the actual expected value, then $z$ ~ N(0, 1)
- Visualize the distribution
- According to the picture, what is the probability that the test statistic $Z$ would take on a value even further away from it's expected value than the current value $z$? Save this probability to the object `p`.
...
error <- sd(stra) / sqrt(length(stra))
...
Okay, now you have the test statistic, which is a statistical summary of the data. Next you can calculate the (infamous) p-value. 

p-value is the probability that, using a given statistical model, a statistical summary of the data would be the same as or more extreme than the actual observed results.
