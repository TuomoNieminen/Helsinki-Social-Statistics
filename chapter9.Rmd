...
In [statistical hypothesis testing](https://en.wikipedia.org/wiki/Statistical_hypothesis_testing) we want to test if our claim or something we assume is supported by the data. To do a statistical test, you need to follow these steps:
...
jokin tehtävä testisuureista + määritelmä

After setting the hypotheses, we can use the data we have and calculate the test statistic. What test statistic to use, depends on the test you are doing.
...
Okay, now you have the test statistics. So next you can calculate the (famous) p-value. The p-value tells you something about the hypotheses and it can be used as a guide line. The p-value and other knowledge of the field you are testing, you can reject or not reject the null hypothesis. 
