...
Welcome. In these next chapters (6-10) we will be using a data set again collected from the students of the faculty of social sciences in the university of Helsinki. The data is from this questionare[link]. The questions have been combined into three combination variables:

- deep (score from questions measuring deep learning)
- stra (score from questions measuring strategic learning)
- surf (score from questions measuing surface learning)

*** =instructions
- Look at the structure of learning2014 with `str()`.
- Look at the first six observations of the learning data
...
# use cut() to code variables as factors
# use table() to get the frequencies of the factors levels
...
# use cut() to code deep learning as a categorical variable (factor)
# use table() to get the frequencies of the factors levels
deep <- table(cut(learning2014$deep, c(1,2,3,4,5), include.lowest = T))

# use table and cut to create object surf here


# draw a bar plot of deep learning
barplot(deep, main = "Distribution of students scores on deep learning")

# draw a bar plot of surface learning
...
# use table and cut to create object surf here
surf <- table(cut(learning2014$surf, c(1,2,3,4,5), include.lowest = T))
...
In R, `T` and `F` can be used to abreviate `TRUE` and `FALSE`. So, for example `c(F, T) & c(T, T)` produces a logical vector `FALSE TRUE` because `F & T` equals `FALSE` and `T & T` equals `TRUE`. 
...
Probability is the measure of the likelihood that an event will occur and it is quantified as a number between 0 and 1. Probability is an **essential** tool for statistics since the discipline is primarily interested in measuring uncertainty.

The classical definition of probability is the proportion of favourable cases over all possible outcomes. Thus, the probability of event $A$ with $f$ number of favourable cases out of $N$ possible outcomes is:
