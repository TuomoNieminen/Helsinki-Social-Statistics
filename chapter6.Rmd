...
The *learning2014* dataset includes these variables along with the students age and gender. The data also includes the students points in a statistics exam.
...
- `1:5` creates an integer vector identical to `c(1,2,3,4,5)`
...
# cut() is used to transform deep into a categorical variable (factor data type)
# table() is used to get the counts of different categories
deep <- table(cut(learning2014$deep, breaks = 1:5, include.lowest = T))

# barplot() is used to visualize the counts computed above
barplot(deep, main = "Distribution of students scores on deep learning")

# Transform the variable attitude into a factor and compute the counts
...
# Use cut() to transform deep into a categorical variable (factor data type)
# Use table to get the counts of different categories
deep <- table(cut(learning2014$deep, breaks = 1:5, include.lowest = T))

# Use barplot to visualize the counts computed above
barplot(deep, main = "Distribution of students scores on deep learning")
...
So, for example `c(F, T) & c(T, T)` produces a logical vector `FALSE TRUE` because `F & T` equals `FALSE` and `T & T` equals `TRUE`. 
...
test_output_contains("c(p1, p2, p3, p4)")
