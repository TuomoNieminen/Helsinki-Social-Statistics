...
What you just saw inside the `subset()` function was  **logical comparison**. Logical comparison creates boolean vectors which can then be used to select subsets of data. 
...
The logical operators work like logical conditions: they compare the elements of vectors one at a time and can produce logical vectors.

So, for example `c(F, T) & c(T, T)` produces a logical vector `FALSE TRUE`. (In R, `T` and `F` can be used to abbreviate `TRUE` and `FALSE`).  

With these tools you are ready to do some serious subsetting.
