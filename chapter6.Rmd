...
In R, `T` and `F` can be used to abbreviate `TRUE` and `FALSE`. So, for example `c(F, T) & c(T, T)` produces a logical vector `FALSE TRUE` because `F & T` equals `FALSE` and `T & T` equals `TRUE`. 

It is also possible to use parenthesis when using multiple conditions, to control the order of the operators. With these tools you are now ready to do some serious subsetting.

*** =instructions
- Create and print out four subsets matching the following conditions: 
- (gender is male) and (deep learning is greater than 4.2) and (points is not zero)
- (age is 25 or age is 26) and (attitude is greater than 3.5)
- (gender is female) and (strategic learning is greater than 4.2)
- (deep learning is more than 4.2) and (points is zero or points is greater than 30)

*** =hint
- The instructions include the correct use of parenthesis when using the logical operators and conditions
...
# male students who scored high on deep learning and have points
...
# female students who scored high on strategic learning
...
test_output_contains("subset(learning2014,(gender=='N')&(stra>4.2))", incorrect_msg="Please print out the subset containing the female students with high stra variable")
test_output_contains("subset(learning2014,deep>4.2&(points==0|points>30))", incorrect_msg="Please print out the subset containing the students who scored high on deep learning and had either no points or very high points")
...
where n() gives the number of outcomes (elements). 
