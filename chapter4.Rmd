...
*Standard deviation* is perhaps the most important measure of variability in statistics. It is defined as the average distance of a set of data points from their mean. So, the closer the values of a variable are to their mean, the smaller the standard deviation.
...
- Create object `x2` similarily setting `sd = 2`. What changed? Compute the sample standard deviation.
...
[Clustering](https://en.wikipedia.org/wiki/Cluster_analysis) is the task of grouping a set of objects in such a way that objects in the same group (called a cluster) are more similar to each other than to those in other groups. [K-means](https://en.wikibooks.org/wiki/Data_Mining_Algorithms_In_R/Clustering/K-Means#Algorithm) is a popular algorithm that performs clustering. With k-means, data points that share the same closest *centroid* are concidered similar.
...
- Print out the values of the centers by typing `km$centers`. Can you find interpretations to the clusters?
- Use `$` to access the vector `cluster` in the object `km`. Give that vector to the `col` argument of `plot()` to visualize the clusters.
...
# Use a trick to plot the students heights with a bit of noise.
...
test_function("kmeans",args=c("x","centers"), not_called_msg = "Please call `kmeans()` with the correct arguments.")
