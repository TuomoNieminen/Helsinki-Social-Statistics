...
[Clustering](https://en.wikipedia.org/wiki/Cluster_analysis) is the task of grouping a set of objects in such a way that objects in the same group (called a cluster) are more similar to each other than to those in other groups. [K-means](https://en.wikibooks.org/wiki/Data_Mining_Algorithms_In_R/Clustering/K-Means#Algorithm) is a popular algorithm that performs clustering. With k-means, data points that share the same closest *centroid* are concidered similar. There are k centroids forming k clusters around them.  
...
*** =instructions
- Use k-means with two centroids (centers).
- Look at the structure of the `km` object. It is of a new type *list*, which is similar to *data.frame*.
- Print out `km$centers`. Can you find interpretations to the clusters?
- Plot the student heights, separated by sex.
- Modify the object `student_colors` to include the vector of clusters found by `kmeans()`. Draw the plot again.
...
*** =sample_code
```{r}
# height and sex are available

# Use k-means to group the students by height
km <- kmeans(height, centers = 1)[1:2]

# Look at the structure of km
str(km)

# Print out the centroid values (centers)


# Plot of the student heights separated by sex.
student_colors <- 1
plot(sex, height, col = student_colors, xaxt = "n", main = "Student heights by sex")
...
# Use k-means to group the students by height
km <- kmeans(height, centers = 2)[1:2]
...
# Plot of the student heights.
student_colors <- km$cluster
plot(sex, height, col = student_colors, xaxt = "n", main = "Student heights by sex")
...
test_output_contains("km$centers")
test_object("student_colors")
...
simulate <- function(n = 60) {
  ...
  y <- rowSums(X) + rnorm(n)
  # scatterplot
  plot(x, y, col = "grey40", type = "p", pch= 19, xlab="", ylab="")
