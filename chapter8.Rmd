...
You can set or get a single value from a vector by using indices and brackets `[ ]`. Using an index number inside the bracets gives access to a single value from the vector. Using a vector of indices gives access to multiple values (another vector). 
...
- Use brackets and indices on `names` to create a new vector `girls` with values "Liisa" and "Anna" (in that order). DO NOT use `girls <- c("Liisa", "Anna")`.
- Use brackets and indices on `names` to create a new vector `boys` with values "Pekka" and "Heikki" (in that order). DO NOT use `boys <- c("Pekka", "Heikki")`
...
Clearly there is uncertainty involved in our point estimate. But how much uncertainty? This is where we need statistical theory. 

One way to approach this question is to try to come up with an interval around the point estimate to describe our uncertainty. It turns out that the following formula produces an interval which contains the true population mean a specified high percentage of the time ($100 \cdot (1-\alpha)$%) 

$$\[\text{mean} - z_{\alpha/2} \cdot \frac{s}{\sqrt{n}}, \text{mean} + z_{\alpha/2} \cdot \frac{s}{\sqrt{n}}\]$$
  
where $s$ is the sample standard deviation, $n$ is the sample size and $z(\frac{\alpha}{2})$ is a critical value from the $N(0,1)$ distribution. The part 

$$\frac{s}{\sqrt{n}}$$

is called the *standard error* of the estimate. 

I know what you're thinking. It's confusing: where did the normal distribution come from, what's with all that $z_{\alpha/2}$ stuff and a critical whatnow? The answers lie in the **sampling distribution** which we will soon get to.


*** =instructions
- Compute the standard error of the sample mean
- Compute the critical value z using a 99% confidence level (alpha = 0.01)
- Compute lower and upper limits of the confidence interval (CI) by applying the formula above.
- Combine the point estimate and the lower and upper CI values to the object `interval_estimate`.
- Print the values in `interval_estimate`, rounded to 1 digit (`round()`)
...
Remember that you can think of the sampling distribution as a kind of mind game:  If we could collect our random sample again and again and calculate a sample statistic each time, the resulting distribution of those statistics would be the sampling distribution of that statistic.
...
- Compute basic summary statistics of the means
...
Käydään läpi mitä viime tehtävässä tapahtui ja muodostetaan yhteys normaalijakaumaan.

*** =instructions
- Take a random sample from learning2014$points and create object `points_sample`
- Compute the mean of the sample and create object `mu_hat`
- Compute the standard deviation of the sample and create object `s`
- Compute the standard error and create object `error`
- Print out the sample mean and it's standard error, rounded to 2 digits
- Inside curve(dnorm()) adjust the parameters for the density of the normal distribution by setting `mean` = the sample mean of your random sample, `sd` = the standard error of the sample mean. 
- Now execute the two lines simultaniously which visualize (1) the previously simulated sampling distribution, and (2) a normal approximation to that distribution based on your single sample.
...
*** =sample_code
```{r}
# learning214, means are available
...
# Histogram of the previously simulated sampling distribution of the sample mean and
# The normal approximation of that sampling distribution, based on a single sample
hist(means, freq = F)
...
# The previously simulated sampling distribution of the sample mean along with
# the normal approximation of the sampling distribution, based on a single sample
hist(means, freq = F)
