...
- Create a new vector `boys` with values `Pekka` and `Matti` from `names` by using indices.
...
# The 2. and 3. value in the vector
names[c(2, 3)]
...
There is quick way to create a vector with integer (whole numbers) values in R. The following two lines produce identical results
...
In programming, often times there is the need to repeat an action multiple times. In statistical programming you might for example have a theory and you wish to explore that theory by simulation.
...
Inside the curly braces are some commands and the current value of the counter can be used there. The commands are repeated until all the values of the vector have been looped through. 

This may seem confusing but don't worry, examples will make it more clear.
...
*** =sample_code
```{r}
# loop through character values and print them
for(letter in c("a","b","c")) {
  print(letter)
  }

# loop through integers, perform and action and print.
for(i in 1:3) {
  print(i+5)
  }
```
...
One simple way to achieve this is to first create an empty vector using for example:

`numeric(10)`

which creates a numeric vector of ten zeroes. The vector can then be filled with new values using indeces.
...
# create a numeric vector and print out thevalues
x <- numeric(N)
x
