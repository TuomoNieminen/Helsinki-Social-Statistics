...
If we could collect our random sample again and again and again and calculate the mean for each of the samples the resulting distribution of these sample means would be the sampling distribution of the sample mean.
...
Remember that you can think of the sampling distribution as a kind of mind game:  If we could collect our random sample again and again and calculate a statistic for each of the samples, the resulting distribution would be the sampling distribution of that statistic.

In reality though, we can't produce this distribution. Or can we? 

Let's pretend for a moment that the data we have actually *is* the target population. Let's see what happens if we repeatedly take samples from that data, and calculate a sample statistic each time, storing the results. This satisfies the idea of the sampling distribution, meaning that we can actually simulate the distribution! 

This method (called [bootstrapping](https://en.wikipedia.org/wiki/Bootstrapping_(statistics))) is widely used in statistics.
...
- Draw a histogram of the sampling disribution of the means.
...
# Repeat N times: 
  # (1) draw a random sample of 50 students
  # (2) compute the mean of exam points and 
