...
$z$ is a quantile point from the $N(0,1)$ distribution. The confidence interval is actually based on a normal approximation of the sampling distribution. In this exercise we'll explore if that approximation seems to work.
...
- Inside `curve(dnorm())`, set `mean` = the estimate for the population mean based on the random sample of 50, `sd` = the standard error of the estimate. 
- Execute the line with `hist()`  and `curve()` which visualize (1) the previously simulated sampling distribution of the means, and (2) a normal approximation to that distribution based on your single sample. (Techically it's actually two lines of code, because `;` is code for line change)
