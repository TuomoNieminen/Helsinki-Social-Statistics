...
boxplot(students2014$ika, main = "Boxplot of student ages", xlab = "age", horizontal = T)
...
In base R graphics, the function `barplot()` can be used to draw a bar plot. The first argument (`height`) should contain the counts you wish to visualize. The counts can be computed with `summary()` or more generally `table()`.
...
- Print out a table of the age categories.
...
- See the previous exercise for more information on how to create a bar plot. The object `age_cut` is very similar to `students2014$kone`.
...
# Print out a table of the students age categories
...
test_output_contains("table(age_cut)",
 incorrect_msg = "Please use `table()` on `age_cut`")
