...
## Brackets have you data[under, control]

It is quite often necessary to access only a part of a dataset. This is called *subsetting* and there are many ways to do it. The best way will depend on the situation and preference.

We have already seen that the column of a data frame (df) can be accessed via the `$`-sign. There is another way: the `[ ]`. The brackets are a general way of accessing a part of any data structure in R. 

The brackets are used by giving the locations of the values you wish to access.


*** =instructions
- Modify the code so that the object age_height contains all the data of the columns three and four of the data frame students2014.
...
# Choose the whole column 3
student_age <- students2014[, 3]

# Choose all rows in the third and fourth columns
age_hight <- students2014[, NULL]
...
--- type:NormalExercise lang:r xp:100 skills:1 key:b26060ee08
...
In this exercise you will draw a barplot of `variable` in the data frame `df`.
