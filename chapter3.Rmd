...
# Create a summary of the numeric 'pituus' in 'students2014'
...
Bar plots are useful for visualizing qualitative, discrete variables. A bar plot represents the counts of the unique values of a variable -  what the function `summary()` shows for factor type objects. Drawing a bar plot only makes sense when there are only a few unique values possible. 
...
- Then give the object os_counts as the first to `barplot()`. 
...
When a variable is continuous by nature, it means that it can take on so many different values that all of it's observed values are likely to be unique.

Age can be interpret as a continuous variable because of the continuous nature of time. The exact age of a person depends on the exact time of birth and is different for all. However, age is often measured quite discretely as rounded down years.

Even when age is measured in years, usually there are too many unique values to draw bar plot that makes sense, which you will soon observe. So what would be a good way of looking at the distribution of age? A solution is to first categorize age.
...
- Follow the procedure in the previous exercise and use `summary()` and then `barplot()` to get and then visualize the counts of unique values of `age_cut`.
...
The plotting functions in R also have many additional arguments which allow for more control over the plot. One simple argument is `col`, the desired color ("blue", "green", etc). Some possible color names can be found <a target="_blank" href="http://www.stat.columbia.edu/~tzheng/files/Rcolor.pdf">here</a>.
...
- Give your plot the main title "Box plot of student ages".
