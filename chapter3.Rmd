...
*** =instructions
- Execute the code that creates a summary of the factor data object `kone` in the data frame `students2014
- Execute the code that creates a summary of the numeric data object `ika` in the data frame `students2014`
- Create a summary of the data.frame object `students2014`.
...
# Create a summary of a factor object (qualitative)
summary(students2014$kone)

# Create a summary of a numeric object (quantitative)
...
When a variable is continuous by nature, it means that it can take on so many different values that all of it's observed values are unique. In practise though, measurements are always discrete and hence continuity is a matter of interpretation.
...
Even when age is measured in years, there are usually too many unique values to draw bar plot that makes sense. A solution is to first categorize age.
