...
- Print the data yet again. YOu have successfully combined variables!
...
- Use the `table()` and `prop.table()` functions to create another proportional table of the variable `kone` from the `students2014` dataset. Save the table in `os_tbl` object.
- Turn the proportional values in `os_tbl` into percentages by executing the last line of the sample. Note that you can multiply the tables values by multiplying the table object! 
...
- you can multiply the tables values by multiplying the table object, for example `os_tbl * 5`.
...
# Make the os_tbl table pretty! From proportional table to percentages:
round(os_tbl*100, digits = 2)
...
# Create a proportional table of 'kone' in 'students2014'
os_tbl <- prop.table(table(students2014$kone))

# Make the os_tbl table pretty! From proportional table to percentages:
round(os_tbl*100, digits = 1)
...
- Create and examine the percentage table `gender_work_pct`. Round the table to make it more reader friendly. Look at the margins of the table. 
...
# Look at the percentage table and add margins to it
gender_work_pct <- prop.table(gender_work_tbl)*100
round(gender_work_pct, digits = 1)
round(addmargins(gender_work_pct), digits = 1)
