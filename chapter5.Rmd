...
- The mean vector we got is the sum of variables. Simple, isn't it?

*** =hint
- The sum of variables can be computed also with sum: the row sums could be used instead of the row means. 
...
*** =instructions
- Execute `gender_work_tbl` and see how the table of variable `sukup` and `toita` looks like. 
...
- Do a better barplot of `gender_os_tbl` by adding a legend. You can add a legend with code `legend=rownames(gender_os_tbl)`. Note that the  `rownames()` is also a function, and it takes the table as an argument.
...
# Barplot of 'gender_os_tbl'
barplot(gender_os_tbl, beside = FALSE, legend=rownames(gender_os_tbl))
...
test_function("barplot", args=c("height", "beside", "legend"), incorrect_msg = "Did you use arguments `beside` and `legend` in the barplot?")
